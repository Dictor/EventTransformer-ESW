...
int8_model_fx = load('best_model_pytorchfx.pt', model)
# int8_model_torch = load('best_model_pytorch.pt', model)
...
# Load sparse matrix
test_events = pickle.load(open("./datasets/ESW/back6/train/back6_0.pckl", 'rb'))  # events (t x H x W x 2)
test_pdls = pickle.load(open("./datasets/ESW/back6/pdl/back6_pedal_0.pckl", 'rb'))  # events (t x H x W x 2)
...
print("clf_logits t : ", time.time()-t)

t = time.time()
int8_test_clf_logits = int8_model_fx(test_pols, test_pixels)   #test_embs, test_clf_logits = 
print("fx_clf_logits t : ", time.time()-t)

# t = time.time()
# torch_test_clf_logits = int8_model_torch(test_pols, test_pixels)   #test_embs, test_clf_logits = 
# print("torch_clf_logits t : ", time.time()-t)

print("test_labels : ", test_labels[0])
print("clf_logits : ", test_clf_logits.argmax())
print("int8_clf_logits : ", int8_test_clf_logits.argmax())

# samples_folder = "./datasets/back6/train/"
# samples = os.listdir(samples_folder)
# for filename in samples:
#     # path = "./datasets/ESW/clean_dataset_1000_old/train/back1_"
#     total_events = pickle.load(open(samples_folder+filename, 'rb'))
...
#     if test_clf_logits[1].argmax()==0:
#         pre_pdl="acc"
#     elif test_clf_logits[1].argmax()==2:
#         pre_pdl="brk"
#     else:
#         pre_pdl = "na"

#     print(f"pred : {pre_pdl} in : {pdl_in}") #, end='\r'   ans : {test_labels[0]} 
